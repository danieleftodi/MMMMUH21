...
{
    DBG_LOG("[DBG] AVL::NodeHeight :: ", "BEGIN :: ", "p: ", std::to_string(p->data));
    int hl;
    int hr;
    ...
    // else return hr + 1
    DBG_LOG("[DBG] AVL::NodeHeight :: ", "END   :: ", "p: ", std::to_string(p->data), " hl: ", std::to_string(hl), " hr: ", std::to_string(hr), " ret: ", std::to_string(hl > hr ? hl + 1 : hr + 1));
    return hl > hr ? hl + 1 : hr + 1;
}

int AVL::BalanceFactor(class Node *p)
{
    DBG_LOG("[DBG] AVL::BalanceFactor :: ", "BEGIN :: ", "p: ", std::to_string(p->data));
    int hl;
    int hr;
    ...
    DBG_LOG("[DBG] AVL::BalanceFactor :: ", "END   :: ", "p: ", std::to_string(p->data), " hl: ", std::to_string(hl), " hr: ", std::to_string(hr), " ret: ", std::to_string(hl - hr));
    return hl - hr;
}
...
{
    DBG_LOG("[DBG] AVL::rInsert :: ", "BEGIN :: key: ", std::to_string(key));
    class Node *t = nullptr;
    
    // INFO :: THIS DOES ALL OF THE CREATION OF NEW NODES, AND ADDING THE key-VALUE TO IT!
    //         [RECURSION 101]
    if (p == nullptr) {
        DBG_LOG("[DBG] AVL::rInsert :: ", "t = new class Node :: key: ", std::to_string(key));
        t = new class Node;
        t->data = key;
        t->lchild = nullptr;
        t->rchild = nullptr;
        t->height = 1; // Starting height from 1 onwards instead of 0
        DBG_LOG("[DBG] AVL::rInsert :: ", "t = new class Node :: ", "END :: key: ", std::to_string(key));
        return t;
    }
    
    if (key < p->data)
    {
        DBG_LOG("[DBG] AVL::rInsert :: ", "if (key < p->data) --> p->lchild :: key: ", std::to_string(key));
        p->lchild = rInsert(p->lchild, key);
    }
    else if (key > p->data)
    {
        DBG_LOG("[DBG] AVL::rInsert :: ", "if (key > p->data) --> p->rchild :: key: ", std::to_string(key));
        p->rchild = rInsert(p->rchild, key);
    }
    
    // Update height
    DBG_LOG("[DBG] AVL::rInsert :: ", "Update height :: key: ", std::to_string(key));
    p->height = NodeHeight(p);
    DBG_LOG("[DBG] AVL::rInsert :: ", "Update height :: key: ", std::to_string(key), " :: h: ", std::to_string(p->height));
    
    // Balance and rotation
    if (BalanceFactor(p) == 2 && BalanceFactor(p->lchild) == 1)
    {
        DBG_LOG("[DBG] AVL::rInsert :: ", "LLRotation :: key: ", std::to_string(key));
        return LLRotation(p);
    }
    else if (BalanceFactor(p) == 2 && BalanceFactor(p->lchild) == -1)
    {
        DBG_LOG("[DBG] AVL::rInsert :: ", "LRRotation :: key: ", std::to_string(key));
        return LRRotation(p);
    }
    else if (BalanceFactor(p) == -2 && BalanceFactor(p->rchild) == -1)
    {
        DBG_LOG("[DBG] AVL::rInsert :: ", "RRRotation :: key: ", std::to_string(key));
        return RRRotation(p);
    }
    else if (BalanceFactor(p) == -2 && BalanceFactor(p->rchild) == 1)
    {
        DBG_LOG("[DBG] AVL::rInsert :: ", "RLRotation :: key: ", std::to_string(key));
        return RLRotation(p);
    }
    
    DBG_LOG("[DBG] AVL::rInsert :: ", "END   :: key: ", std::to_string(key));
    return p;
}
...
int one_main()
{
    ...
    {
        DBG_LOG("\n[DBG] one_main :: ", "rInsert :: " , "FOR-Loop :: i: ", std::to_string(i), " key: ", std::to_string(A[i]));
        tree.root = tree.rInsert(tree.root, A[i]);
    }

    // Inorder Traversal
    DBG_LOG("\n[DBG] one_main :: ", "InOrder :: " , "");
